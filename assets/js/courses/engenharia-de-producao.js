...
// --- DOM ELEMENTS ---
        ...
        function saveState() {
            ...
            storage.setItem('skillTreeProgress_Producao', JSON.stringify(stateToSave));
        ...
        function loadState() {
            ...
            const savedStateJSON = storage.getItem('skillTreeProgress_Producao');
        ...
        function renderOptionalTrackHeaders() {
            Object.entries(SPECIALIZATION_TRACKS).forEach(([trackName, subjectIds]) => {
                const trackNodes = optionalNodes.filter(node => subjectIds.includes(node.id) && typeof node.y === 'number');
                ...
                const yPos = Math.max(3, Math.min(96, (minY + maxY) / 2));

                const header = document.createElement('div');
                header.className = 'track-header';
                header.textContent = `Trilha: ${trackName}`;
            ...
            do {
                ...
                nodes.forEach(subject => {
                    ...
                    const allDependenciesMet = subject.dependencies.every(depId => {
                        const requiredPeriod = parsePeriodDependency(depId);
                        if (requiredPeriod !== null) return currentPeriod >= requiredPeriod;
        ...
        function updateAllProgressBars() {
            ...
            const completedMandatoryHours = coreNodes
                .filter(s => s.state === 'subject-completed' || s.state === 'subject-satisfied')
                .reduce((sum, s) => sum + s.cht, 0);
            const totalProgress = (completedMandatoryHours / TOTAL_MANDATORY_HOURS) * 100;
            totalProgressBar.style.width = `${Math.min(totalProgress, 100)}%`;
            totalProgressText.textContent = `${completedMandatoryHours}/${TOTAL_MANDATORY_HOURS}h (${totalProgress.toFixed(1)}%)`;
        ...
        // --- Settings Logic ---
        let isGlowEnabled = storage.getItem('skillTreeGlowEnabled_Producao');
        ...
        glowToggle.addEventListener('change', (e) => {
            if (e.target.checked) {
                ...
                storage.setItem('skillTreeGlowEnabled_Producao', 'true');
            } else {
                document.body.classList.add('disable-glow');
                storage.setItem('skillTreeGlowEnabled_Producao', 'false');
            }
        });

        resetButton.addEventListener('click', () => {
            // Remove os dados salvos para garantir um reset completo
            storage.removeItem('skillTreeProgress_Producao');
        ...
        function checkAndCelebrate() {
            ...
            // Mantém os requisitos de conclusão definidos originalmente para Produção.
