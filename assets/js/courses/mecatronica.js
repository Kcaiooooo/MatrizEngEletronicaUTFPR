...
const storage = window.KMStorage.forCourse({"progressKey":"skillTreeProgress_Mecatronica_mecatronica","legacyProgressKey":"skillTreeProgress_Mecatronica","glowKey":"skillTreeGlowEnabled_Mecatronica_mecatronica","legacyGlowKey":"skillTreeGlowEnabled_Mecatronica"});
        ...
        function saveState() {
            ...
            storage.setItem('skillTreeProgress_Mecatronica', JSON.stringify(stateToSave));
        ...
        function loadState() {
            ...
            const savedStateJSON = storage.getItem('skillTreeProgress_Mecatronica');
        ...
        function renderOptionalTrackHeaders() {
            const trackLabels = {
                '[1139]': 'Trilha [1139]: Automação e Sistemas',
                '[1138]': 'Trilha [1138]: Modelagem e Controle',
                '[1148]': 'Trilha [1148]: Manutenção Industrial',
                '[1149]': 'Trilha [1149]: Proj. Eletrônicos E Máquinas',
                '[1147]': 'Trilha [1147]: Energia e Sustentabilidade',
                '[1146]': 'Trilha [1146]: Instal. e Eficiência Energ.'
            };
            const trackOrder = ['[1139]', '[1138]', '[1148]', '[1149]', '[1147]', '[1146]'];
        ...
        // --- Settings Logic ---
        let isGlowEnabled = storage.getItem('skillTreeGlowEnabled_Mecatronica');
        ...
        glowToggle.addEventListener('change', (e) => {
            if (e.target.checked) {
                ...
                storage.setItem('skillTreeGlowEnabled_Mecatronica', 'true');
            } else {
                document.body.classList.add('disable-glow');
                storage.setItem('skillTreeGlowEnabled_Mecatronica', 'false');
            }
        });

        resetButton.addEventListener('click', () => {
            // Remove os dados salvos para garantir um reset completo
            storage.removeItem('skillTreeProgress_Mecatronica');
