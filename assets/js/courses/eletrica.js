import { TOTAL_COMPLEMENTARY_HOURS, TOTAL_EXTENSION_HOURS, TOTAL_HUMANITIES_HOURS, TOTAL_OPTIONAL_HOURS, NODE_WIDTH, NODE_HEIGHT, SPECIALIZATION_TRACKS, OPTIONAL_GROUPS_CONFIG, allNodesData, allHumanitiesData, allOptionalNodesData } from '../data/eletrica.js';
...
const storage = window.KMStorage.forCourse({"progressKey":"skillTreeProgress_Eletrica","legacyProgressKey":"skillTreeProgress_Eletrica","glowKey":"skillTreeGlowEnabled_Eletrica","legacyGlowKey":"skillTreeGlowEnabled_Eletrica"});
        ...
        function saveState() {
            ...
            storage.setItem('skillTreeProgress_Eletrica', JSON.stringify(stateToSave));
        ...
        function loadState() {
            ...
            const savedStateJSON = storage.getItem('skillTreeProgress_Eletrica');
        ...
        function renderOptionalTrackHeaders() {
            const trackPositions = {
                ...
                'Trilha [1152]: Manutenção Industrial': 40,
                'Trilha [1153]: Proj. Eletrônicos E Máquinas': 55,
                'Trilha [1144]: Energia e Sustentabilidade': 70,
                'Trilha [1143]: Instal. e Eficiência Energ.': 85,
            };
        ...
        function updateAllProgressBars() {
            ...
            const coreNodes = nodes.filter(n => n.groupId !== '[1142]');
            ...
            const totalCompletedHumanities = humanitiesNodes
                .filter(n => n.state === 'humanities-completed' && n.groupId !== '[1142]')
                .reduce((sum, n) => sum + n.cht, 0);
            ...
            // 5. Progresso Grupo 1142 (Empregabilidade e Empreendedorismo)
            if (group1142ProgressBar && group1142ProgressText) {
                const group1142Nodes = humanitiesNodes.filter(n => n.groupId === '[1142]' && n.state === 'humanities-completed');
                const group1142Hours = group1142Nodes.reduce((sum, n) => sum + n.cht, 0);
                const group1142Req = OPTIONAL_GROUPS_CONFIG['[1142]'] ? OPTIONAL_GROUPS_CONFIG['[1142]'].requiredHours : 135;
        ...
        // --- Settings Logic ---
        let isGlowEnabled = storage.getItem('skillTreeGlowEnabled_Eletrica');
        ...
        glowToggle.addEventListener('change', (e) => {
            if (e.target.checked) {
                ...
                storage.setItem('skillTreeGlowEnabled_Eletrica', 'true');
            } else {
                document.body.classList.add('disable-glow');
                storage.setItem('skillTreeGlowEnabled_Eletrica', 'false');
            }
        });

        resetButton.addEventListener('click', () => {
            // Remove os dados salvos para garantir um reset completo
            storage.removeItem('skillTreeProgress_Eletrica');
