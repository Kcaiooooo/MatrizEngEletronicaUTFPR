import { TOTAL_HUMANITIES_HOURS, TOTAL_OPTIONAL_HOURS, NODE_WIDTH, NODE_HEIGHT, SPECIALIZATION_TRACKS_M2, OPTIONAL_GROUPS_CONFIG, allHumanitiesData, allOptionalNodesData, allNodesData } from '../data/radiologia.js';
import { parseProgress, renderActivityList as renderSafeActivityList } from '../shared/progress.js';
const storage = window.KMStorage.forCourse({"progressKey":"skillTreeProgress_Radiologia","legacyProgressKey":"skillTreeProgress_Radiologia","glowKey":"skillTreeGlowEnabled_Radiologia","legacyGlowKey":"skillTreeGlowEnabled_Radiologia"});
            ...
            function saveState() {
                ...
                storage.setItem('skillTreeProgress_Radiologia', JSON.stringify(stateToSave));
            ...
            function loadState() {
                ...
                const savedStateJSON = storage.getItem('skillTreeProgress_Radiologia');
            ...
            currentPeriod = 1;
                ...
                // Garante estados padrão iniciais para nós sem estado
                nodes.forEach(node => {
                    if (!node.state) node.state = node.dependencies.length === 0 ? 'subject-available' : 'subject-locked';
                });
                optionalNodes.forEach(node => {
                    if (!node.state) node.state = node.dependencies.length === 0 ? 'optional-available' : 'optional-locked';
                });
                ...
                // TENTA CARREGAR O ESTADO SALVO
                    ...
                    optionalNodes.forEach(node => {
                        node.state = node.dependencies.length === 0 ? 'optional-available' : 'optional-locked';
                    });
            ...
            function renderOptionalTrackHeaders() {
                ...
                const trackNodes = optionalNodes.filter(node => typeof node.y === 'number');
                const minY = trackNodes.length > 0 ? Math.min(...trackNodes.map(node => node.y)) : 10;
                const maxY = trackNodes.length > 0 ? Math.max(...trackNodes.map(node => node.y)) : 70;
                const yPos = Math.max(25, Math.min(75, (minY + maxY) / 2));

                const header = document.createElement('div');
                header.className = 'track-header';
                header.textContent = 'Disciplinas Optativas (Humanísticas e Profissionais)';
            ...
            function getGroupProgress(groupId) {
                ...
                // For group [1099] include humanitiesNodes and optionalNodes as well
                const allSearchable = [...nodes, ...humanitiesNodes, ...optionalNodes];
                const subjectsInGroup = allSearchable.filter(n => n.groupId === groupId);
                const completedHours = subjectsInGroup
                    .filter(n => n.state === 'subject-completed' || n.state === 'humanities-completed' || n.state === 'optional-completed')
            ...
            function renderNodes(container, data) {
                data.forEach(node => {
                    ...
                    nodeEl.className = `node ${node.state || (node.type === 'optional' ? 'optional-available' : 'subject-available')}`;
                ...
                if (node.dependencies && node.dependencies.length > 0) {
                    const depColors = node.dependencies
                        .filter(d => !d.startsWith('Periodo:'))
                        .map(depId => getColorForNodeId(depId));
            ...
            function updateAllProgressBars() {
                // Main mandatory progress (Total 2565h in 35 subjects)
                const coreNodes = nodes;
                const completedCoreHours = coreNodes
                    .filter(s => s.state === 'subject-completed' || s.state === 'subject-satisfied')
                    .reduce((sum, s) => sum + s.cht, 0);
                const totalProgress = (completedCoreHours / 2565) * 100;
                totalProgressBar.style.width = `${Math.min(totalProgress, 100)}%`;
                totalProgressText.textContent = `${totalProgress.toFixed(1)}% (${completedCoreHours}/2565h)`;
                ...
                // Single Combined Optativas Progress Bar (Group [1099] requirement: 60h total across humanísticas & profissionais)
                const completedOptionalHours = optionalNodes
                    .filter(n => n.state === 'optional-completed')
                    .reduce((sum, n) => sum + n.cht, 0);

                const completedHumanitiesHours = humanitiesNodes
                    .filter(n => n.state === 'humanities-completed')
                    .reduce((sum, n) => sum + n.cht, 0);

                const totalCompletedOptativas = completedOptionalHours + completedHumanitiesHours;
                const optativasProgress = (totalCompletedOptativas / 60) * 100;

                if (optionalProgressBar && optionalProgressText) {
                    optionalProgressBar.style.width = `${Math.min(optativasProgress, 100)}%`;
                    optionalProgressText.textContent = `${totalCompletedOptativas}/60h (${optativasProgress.toFixed(1)}%)`;
                }
            ...
            // --- Settings Logic ---
            let isGlowEnabled = storage.getItem('skillTreeGlowEnabled_Radiologia');
            ...
            glowToggle.addEventListener('change', (e) => {
                if (e.target.checked) {
                    ...
                    storage.setItem('skillTreeGlowEnabled_Radiologia', 'true');
                } else {
                    document.body.classList.add('disable-glow');
                    storage.setItem('skillTreeGlowEnabled_Radiologia', 'false');
                }
            });
            ...
            resetButton.addEventListener('click', () => {
                ...
                storage.removeItem('skillTreeProgress_Radiologia');
            ...
            */
            function checkAndCelebrate() {
                ...
                // --- RADIOLOGIA: Requisitos de conclusão ---
                // 1. Todas as obrigatórias concluídas
                const completedMandatory = nodes.filter(s => s.state === 'subject-completed' || s.state === 'subject-satisfied').length;
                const allMandatoryDone = completedMandatory >= nodes.length;

                // 2. 60h de optativas (humanísticas e profissionais) concluídas
                const totalOptativas = [...optionalNodes, ...humanitiesNodes]
                    .filter(n => n.state.endsWith('-completed'))
                    .reduce((sum, n) => sum + n.cht, 0);
                const optativasDone = totalOptativas >= 60;

                console.log(`Status Tecnólogo em Radiologia:
            - Obrigatórias: ${completedMandatory}/${nodes.length} ${allMandatoryDone ? '✓' : '...'}
            - Optativas: ${totalOptativas}/60h ${optativasDone ? '✓' : '...'}`);

                // --- DECISÃO FINAL ---
                if (allMandatoryDone && optativasDone) {
                    celebrationModal.classList.remove('hidden');
                    try { celebrationAudio.play(); } catch(e) {}
