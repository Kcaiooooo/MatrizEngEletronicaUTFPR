...
const storage = window.KMStorage.forCourse({"progressKey":"skillTreeProgress_M3","legacyProgressKey":"skillTreeProgress_M3","glowKey":"skillTreeGlowEnabled_M3","legacyGlowKey":"skillTreeGlowEnabled_M3"});
        ...
        const group1177ProgressBar = document.getElementById('group1177-progress-bar');
        const group1177ProgressText = document.getElementById('group1177-progress-text');
        ...
        function saveState() {
            ...
            storage.setItem('skillTreeProgress_M3', JSON.stringify(stateToSave));
        ...
        function loadState() {
            ...
            const savedStateJSON = storage.getItem('skillTreeProgress_M3');
        ...
        function renderOptionalTrackHeaders() {
            const trackPositions = {
                ...
                'Trilha de Eng. Biomédica': 26,
                'Trilha de Sist. Computacionais': 38,
                'Trilha de Proc. de Sinais': 51,
                'Trilha de Internet das Coisas': 63,
                'Optativas Gerais': 79,
            };
            ...
            data.forEach(node => {
                ...
                if (node.equivalents && node.equivalents.length > 0) {
                    const equivalentText = node.equivalents
                        .map(equivalent => `${equivalent.id}${equivalent.cht ? ` (${equivalent.cht}h)` : ''}`)
                        .join(', ');
        ...
        function updateAllProgressBars() {
            ...
            const coreNodes = nodes.filter(n => n.groupId !== '[1177]');
            ...
            // 5. Progresso Grupo 1177 (Adm, Empreend e Economia)
            if (group1177ProgressBar && group1177ProgressText) {
                const group1177Nodes = nodes.filter(n => n.groupId === '[1177]' && n.state === 'subject-completed');
                const group1177Hours = group1177Nodes.reduce((sum, n) => sum + n.cht, 0);
                const group1177Req = OPTIONAL_GROUPS_CONFIG['[1177]'] ? OPTIONAL_GROUPS_CONFIG['[1177]'].requiredHours : 90;
                const group1177ProgressVar = group1177Req > 0 ? (group1177Hours / group1177Req) * 100 : 0;
                group1177ProgressBar.style.width = `${Math.min(group1177ProgressVar, 100)}%`;
                group1177ProgressText.textContent = `${group1177Hours}/${group1177Req}h (${group1177ProgressVar.toFixed(1)}%)`;
            }
        ...
        // --- Settings Logic ---
        let isGlowEnabled = storage.getItem('skillTreeGlowEnabled_M3');
        ...
        glowToggle.addEventListener('change', (e) => {
            if (e.target.checked) {
                ...
                storage.setItem('skillTreeGlowEnabled_M3', 'true');
            } else {
                document.body.classList.add('disable-glow');
                storage.setItem('skillTreeGlowEnabled_M3', 'false');
            }
        });
        ...
        resetButton.addEventListener('click', () => {
            ...
            storage.removeItem('skillTreeProgress_M3');
